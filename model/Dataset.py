...
class DataSet(object):
    def __init__(self, images, features, labels, sample='histogram'):
        self.sample = sample
        if sample == 'histogram':
            assert images[0].shape[0] == labels.shape[0], (
                'images.shape: %s labels.shape %s' % (images[0].shape, labels.shape)
            )
            assert images[1].shape[0] == labels.shape[0], (
                'images.shape: %s labels.shape %s' % (images[1].shape, labels.shape)
            )
        else:
